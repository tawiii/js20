...
// 1
  var skill = _.map(parse, 'skills');
  var massive = [];
     _(skill).forEach(function(value) {
        massive = _.union(massive, value);
      });
      function sort(a, b){
        a = a.toLowerCase()
        b = b.toLowerCase()
...
// 2
  var sortik = _.sortBy(parse, 'friends.length');
  var name = _.map(sortik, 'name');
  console.log(name);

// 3
  var mas = [];
  var fr = _.map(parse, 'friends');
  var flat = _.flatten(fr);
  var map = _.map(flat, 'name');
  mas = _.union(map);
  console.log(mas);
